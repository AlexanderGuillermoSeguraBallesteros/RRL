...
## UTILITY FUNCTION ##
def ObjectiveFunction(theta, X, T, M, mu, transactionCost, startPos):
    # Compute the Sharpe ratio #
    F = ComputeF(theta, X, T, M, startPos) # Compute the values of vector of with current theta values.
    #print(F,F.size)
...
## GRADIENT ##
def GradientFunctionM(theta, X, T, M, mu, transactionCosts, startPos):
    ...
    A = rewards.sum() / T # Compute the mean.
    B = np.sum(np.power(rewards, 2)) / T # Compute the stardar deviation.

    dStdA = B/math.pow((B-math.pow(A, 2)), (3/2))
    dStdB = -0.5*(A/(B-math.pow(math.pow(A,2), (3/2))))
    dAdRt = 1
    dBdRt = 2*A

    for t in range(1,T+1):
        dRtdFt = -mu*transactionCosts*np.sign(F[t] - F[t-1])
        dRtdFtp = mu*X[(t - 1) + startPos - M] + mu*transactionCosts*np.sign(F[t] - F[t-1])

        gradient[:,t] = (dStdA * dAdRt + dStdB + dBdRt) * (dRtdFt * dFt[:,t] + dRtdFtp * dFt[:,t - 1])

#        gradient[t] = (dStdA*dAdRt + dStdB+dBdRt)*(dRtdFt*dFt[t] + dRtdFtp*dFt[t-1])

    return -1*gradient[:,T]

def train(theta, X, T, M, mu, transactionCosts, startPos, rho, N):
    sharpe = 1
    previousSharpe = 0
    ...
    for i in range(N):
        ...
        gradient = -1*GradientFunctionM(theta, X, T, M, mu, transactionCosts, startPos)
